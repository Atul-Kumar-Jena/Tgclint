<template>
  <aside class="quote" :aria-hidden="!quoteOpen" @keydown.esc="quoteOpen = false">
    ...
    <div class="quote__panel" role="dialog" aria-modal="true" aria-label="Get a quote" tabindex="-1">
      <div class="quote__sidebar" aria-hidden="true"><span class="quote__sidebar-word">Saansud</span></div>
      <div class="quote__content" data-lenis-prevent>
        ...
        <header class="quote__head">
          ...
          <p class="quote__intro">Tell us about your plot and your plans — we'll reply with a clear estimate and timeline.</p>
        </header>
        <form v-if="!done" class="quote__form" novalidate @submit.prevent="submit">
          ...
          <fieldset class="quote__field quote__chips">
            ...
            <label class="chip"><input type="checkbox" value="Construction"><span>Home construction</span></label>
            <label class="chip"><input type="checkbox" value="Plots"><span>Plots</span></label>
            <label class="chip"><input type="checkbox" value="Interiors"><span>Interiors</span></label>
          ...
          <div class="quote__field">
            ...
            <textarea id="q-msg" rows="5" placeholder="A few words about the plot, the budget and the home you have in mind&hellip;" />
          </div>
...
// halt the page behind the scrim and hand the wheel to the panel —
// Lenis would otherwise swallow wheel events and the form could never scroll
const nuxtApp = useNuxtApp() as any
watch(quoteOpen, (open) => {
  const l = nuxtApp.$lenis
  if (open) { l?.stop(); document.body.classList.add('is-locked') }
  else { l?.start(); document.body.classList.remove('is-locked') }
})
function submit() {
