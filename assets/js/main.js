...
(() => {
  ...
  if (menu && menuBtn) {
    ...
    const setMenu = (open) => {
      ...
      if (open) {
        setTimeout(() => { const f = menu.querySelector("a[href], button"); if (f) f.focus(); }, 80);
      } else {
        menuBtn.focus();
      }
    };
  ...
  $$("[data-rotator]").forEach((rot) => {
    ...
    rot.appendChild(liveEl);
    let i = 0, timer = null;
    const show = (n) => {
      i = (n + slides.length) % slides.length;
      slides.forEach((s, k) => s.classList.toggle("active", k === i));
      dots.forEach((d, k) => d.setAttribute("aria-selected", String(k === i)));
      const heading = slides[i].querySelector("h1,h2,.t-hero");
      liveEl.textContent = heading ? `Slide ${i + 1}: ${heading.textContent.trim()}` : `Slide ${i + 1} of ${slides.length}`;
    };
    ...
    rot.addEventListener("keydown", (e) => {
      if (e.key === "ArrowLeft") { stop(); show(i - 1); play(); }
      else if (e.key === "ArrowRight") { stop(); show(i + 1); play(); }
    });
